...
Construct a scatterplot of `hwy` versus `cty` with the color of the points mapped to `drv`.
...
Now construct second scatterplot of `hwy` versus `cty`, but this time set the color of the points to your favorite color^[Try typing `colors()` in the console to get a list of named colors.] and `facet` by `drv`. Read `?facet_wrap` and adjust the `ncol` and `scales` as necessary.
...
Construct a scatterplot of `hwy` vs `cty` with a third numerical variable being mapped to `color`; then to `size`; then to `shape`. 
...
Construct a barplot of `cut` and include `fill = carat > 0.7` as an aesthetic mapping. Describe what the mapping fill to `carat > 0.7` does.
